 /* -------------------------
                ...
                messages: [
                    {
                        id: 'm1',
                        from: 'them',
                        text: 'Alo a zai giao dịch trung gian giúp em ạ',
                        time: Date.now() - 1000 * 60 * 60
                    },
                    {
                        id: 'm2',
                        from: 'them',
                        text: 'Em tạo box rồi ạ',
                        time: Date.now() - 1000 * 60 * 60
                    },
    ...
    function sendMessageToBoxGDTG(sender, text, avatar = '') {
// Tìm box GDTG
    const box = demoConvos.find(c => c.id === 'c2');
    if (!box) return console.warn("Box GDTG chưa tồn tại");

    // Tạo message
    const msg = {
        id: 'm' + Math.random().toString(36).slice(2),
        from: sender, // 'Người A' hoặc 'Người B'
        text: text,
    ...
    // Render nếu đang mở
    if (selectedConvo && selectedConvo.id === box.id) {
        renderMessages();
    }
    ...
    // Sửa renderMessages để hiển thị avatar
    function renderMessages() {
    if (!selectedConvo) {
    messagesEl.innerHTML = '<div class="text-center text-xs text-gray-400">Chọn cuộc trò chuyện để bắt đầu</div>';
    return;
    }
    ...
    selectedConvo.messages.forEach(m => {
        const wrap = document.createElement('div');
        wrap.className = 'flex items-start gap-2 mb-2 ' + (m.from === 'me' ? 'justify-end' : 'justify-start');

        // Avatar
        const avatarEl = document.createElement('div');
        avatarEl.className = 'w-8 h-8 rounded-full overflow-hidden flex-shrink-0';
        avatarEl.innerHTML = m.avatar 
            ? `<img src="${m.avatar}" class="w-full h-full object-cover rounded-full">`
            : `<div class="w-full h-full bg-gray-700 flex items-center justify-center text-xs text-white font-semibold">${m.from[0]}</div>`;

        // Bubble
        const bubble = document.createElement('div');
        bubble.className =
            (m.from === 'me' ? 'bg-indigo-600 text-white' : 'bg-gray-700 text-gray-100') +
            ' p-3 rounded-lg max-w-[70%] shadow';
